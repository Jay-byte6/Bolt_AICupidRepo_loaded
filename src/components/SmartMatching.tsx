...
import { findCompatibleMatches, findMatchByCupidId } from '../services/matchingService';
...
const SmartMatching = () => {
  ...
  const findMatches = async (includeLowMatches = false) => {
    ...
    try {
      const matches = await findCompatibleMatches(profile.cupidId, includeLowMatches);
  ...
  const handleCupidIdSearch = async () => {
    ...
    try {
      ...
      if (match) {
        ...
        setError('No registered user found with this CUPID ID');
      }
    } catch (error: any) {
